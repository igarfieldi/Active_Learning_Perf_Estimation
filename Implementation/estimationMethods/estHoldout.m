# usage: [holdoutAccs, iterAccs] = estHoldout(classifier, oracle, holdoutSize)

function [holdoutAccs, iterAccs] = estHoldout(classifier, oracle, holdoutSize)
    ...
    elseif(!isa(classifier, "classifier") || !isa(oracle, "oracle")
            || !isscalar(holdoutSize))
        error("estimationMethods/estHoldout: requires classifier, oracle, scalar");
    endif

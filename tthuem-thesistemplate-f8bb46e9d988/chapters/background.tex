...
A very similar estimate is the \textbf{Bayesian information criterion}(BIC). Defined as $BIC = -2 \cdot log(likelihood) + \lambda log(n)$, it is asymptotically equal to AIC, but with a steeper penalty for complex models due to the exchange of the factor 2 with $log(n)$ \cite{Weakliem1999}. Using the same assumptions as for AIC, we get the formula
\begin{equation}
